...
async function runTest() {
  ...
  try {
    ...
    // Dùng file guests-duplicates.csv
    const csvPath = path.join(__dirname, '../../data/sample-csv/guests-duplicates.csv');
    let content = fs.readFileSync(csvPath, 'utf-8');
    
    // Thêm dummy row để đổi mã hash (bảo đảm không bị lỗi duplicate file nếu chạy nhiều lần)
    content += `\nTest Duplicate Upload ${Date.now()},,GA`;
    const buffer = Buffer.from(content);

    const file = {
      originalname: `guests-duplicates-${Date.now()}.csv`,
      mimetype: 'text/csv',
      buffer: buffer,
    };

    console.log(`Bắt đầu test upload file trùng lặp (duplicates) cho Concert: ${concert.id}...`);
