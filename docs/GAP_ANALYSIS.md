...
| 16 | VIP guest list from nightly CSV, periodic import | ✅ done | Upload endpoint và `InboxPollerService` (`@Cron` mỗi 10 giây) dùng chung BullMQ pipeline. Inbox hỗ trợ processed/failed, composite checksum dedup theo `(concertId, checksum)`, row-level validation, malformed-header retry/FAILED và cleanup khi enqueue lỗi. Đã verify trực tiếp qua Docker; chi tiết tại `COMPLETION_PLAN.md` §4.4 và `blueprint/specs/csv-ingestion.md`. |
